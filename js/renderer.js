...
function _applyCellContent(div, col, row, fs) {
  ...
  if (S.karel.x === col && S.karel.y === row) {
    ...
    if      (c === 'P') { div.classList.add('c-p'); div.innerHTML = K.KAREL_ASSETS.CORALL; }
    else if (c === 'A') { div.classList.add('c-a'); div.innerHTML = K.KAREL_ASSETS.BOMBOLLA; }
    else                { div.classList.add('c-e'); div.innerHTML = ''; }
  }
}

// ── Extreu col/row del target o del seu pare .cell ──
...
// ── Event delegation: un sol listener per mousedown/mouseenter ──
// Es crida una sola vegada des de initRenderer().
function _initGridDelegation() {
  ...
  g.addEventListener('mousedown', e => {
    ...
    if (!c) return;
    e.preventDefault();
    K.state.editDragging = true;
    K.applyBrushAt(c.col, c.row);
  });

  g.addEventListener('mouseenter', e => {
    if (!K.state.editMode || !K.state.editDragging) return;
    const c = _cellFromEvent(e);
    if (c) K.applyBrushAt(c.col, c.row);
  }, true);  // capture phase: mouseenter no fa bubble, cal capture
}
...
K.initGridDelegation = _initGridDelegation;
