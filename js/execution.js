...
function execAction(step) {
  ...
  switch (action) {
    case 'move':
      if (K.isWall(fx, fy)) return errStop('wall');
    ...
    case 'turn-around':
      ...
      break;
    case 'grab':
      if (K.getCell(fx, fy) !== 'A') return errStop('no_water');
      K.setCell(fx, fy, '.'); S.karel.motxilla++;
      K.log(`${cmd} 🫧 → ${S.karel.motxilla}`, 'ok');
      break;
    case 'drop':
      if (S.karel.motxilla <= 0) return errStop('bag_empty');
      if (K.getCell(S.karel.x, S.karel.y) === 'A') {
        K.log(`⚠ ${cmd}: ja hi ha una bombolla aquí — la teva es conserva`, 'inf');
        break;
      }
