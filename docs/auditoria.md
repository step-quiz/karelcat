...
>
> El document anterior `docs/BRIEFING.md` descriu *què* fa el projecte i *quines* tasques hi ha. Aquest document se centra en *com* funciona internament i *on* cal anar amb compte.
...
style.css           — 691 línies. Sense zombies des de la neteja (Categoria C).
...
### 8.1 Capítol 10 del curs (prioritat alta)

Els capítols 1 a 9 existeixen i funcionen (`curs/capitol-1.html` … `curs/capitol-9.html`) i tots estan llistats a `DISPONIBLES` a `curs/index.html`. Únicament el **capítol 10** ("Reptes") és pendent: apareix a `capitols.js` però no té fitxer HTML i es mostra com a no disponible a l'índex.

Hi ha un guió detallat a `docs/guio-capitols-3-10.md` que inclou el contingut del capítol 10.

**Com crear un capítol nou:**
1. Copia `curs/capitol.html` com a `curs/capitol-N.html`.
2. Canvia la constant `CURRENT_CAPITOL` al fitxer.
3. Omple el contingut seguint l'estructura dels capítols existents.
4. Afegeix el nom del fitxer al conjunt `DISPONIBLES` a `curs/index.html`.

Els simuladors s'incrusta via `<iframe src="../index.html?embed=1&map=BASE64&code=BASE64...">`. Per generar les URLs, usa `btoa(encodeURIComponent(text))` al revés de `decodeURIComponent(escape(atob(b64)))` que usa `main.js`.
